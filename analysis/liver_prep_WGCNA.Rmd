...
Filter out genes with very low variance
...
```{r}
mat_expr_combat = ComBat(dat=mat_expr, 
...
```{r}
seuratObj_combat <- CreateSeuratObject(counts = mat_expr_combat, 
                                       assay = "SCT_combat", 
...
seuratObj_combat <- FindVariableFeatures(seuratObj_combat, assay = "SCT_combat")

seuratObj_combat <- ScaleData(object = seuratObj_combat, 
                              assay = "SCT_combat",
                              min.cells.to.block = 25000)

seuratObj_combat<- RunPCA(seuratObj_combat, 
                    assay = "SCT_combat",
                    npcs = npcs, 
                    seed.use=randomSeed,
                    verbose = FALSE)

seuratObj_combat <- RunUMAP(seuratObj_combat,
                            dims=1:npcs,
                            reduction = "pca",
                            assay=  "SCT_combat",
                            seed.use = randomSeed)
...
```{r}
DimPlot(object = seuratObj_combat, 
        1:2, 
        assay=  "SCT_combat",
        reduction = "pca", 
        group.by = "sample_ID",
...
p <- DimPlot(object = seuratObj_combat, 
        1:2, 
        assay=  "SCT_combat",
        reduction = "umap", 
        group.by = "sample_ID",
...
```{r}
p1 <- DimPlot(object = seuratObj_combat, 
        1:2, 
        assay=  "SCT_combat",
        reduction = "umap", 
...
Data looks MUCH better than before! Ready for WGCNA and NMF gene network analysis!

Save ComBat corrected object

```{r}
pathSeuratObj_combat = "/nfsdata/projects/jonatan/pub-perslab/18-liver-fred/output/liver_perslab_int_seurat_7_SCTint_perslab_labels_seuratObj_combat.RDS.gz"

saveMeta(savefnc = saveRDS, object =seuratObj_combat, 
         file = pathSeuratObj_combat, compress="gzip")
```
