...
list_dt_condition_coef = lapply(list_dt_condition_coef, function(dt_condition_coef){
  ...
  vec_celltype <- dt_geneMod_perslab$cell_cluster_merged[sapply(dt_condition_coef$module, function(eachmod) grep(eachmod, dt_geneMod_perslab$module_merged)[1])] 
  dt_condition_coef[["celltype"]] = vec_celltype
  
  # order rows
  dt_condition_coef = dt_condition_coef[order(celltype,module)]
  orderToUse1 = 1:length(vec_celltype)
  # force row order with factors
  dt_condition_coef[["celltype"]] = factor(dt_condition_coef[["celltype"]], ordered = T)
  dt_condition_coef[["celltype"]] = reorder(x=dt_condition_coef[["celltype"]], X = orderToUse1, order=T)
  
  dt_condition_coef[["module"]] = factor(gsub("perslab_","",dt_condition_coef[["module"]]), ordered = T)
  dt_condition_coef[["module"]] = reorder(x=dt_condition_coef[["module"]], X = orderToUse1, order=T)
  # color bars by cell type
  vec_color = sample(x=unique(gsub("\\d","",colors())), size=length(unique(vec_celltype)), replace=F)
  freqs = table(dt_condition_coef[["celltype"]])

  vec_color_celltype = rep(x = vec_color, times = freqs) %>% factor(.,ordered=T)
  dt_condition_coef[["color"]] = vec_color_celltype
  return(dt_condition_coef)
})
```

```{r}
names(list_dt_condition_coef) = c("steatosis","lob_inflam", "fibrosis")
```

plot 

```{r}
lapply(c("steatosis","lob_inflam", "fibrosis"), function(condition) {
  dt_condition_coef = list_dt_condition_coef[[condition]]
  p <- 
    ggplot(data=dt_condition_coef, aes(x=module, y=estimate, fill=celltype)) +
    geom_bar(stat="identity", position=position_identity(), group=dt_condition_coef$celltype) + 
    geom_errorbar(aes(ymin=estimate-std.error, ymax=estimate+std.error), width=.1) +
    scale_fill_brewer(palette="Paired")+
    #scale_fill_manual(values=dt_condition_coef$color) + # this is a kluge
    #scale_fill_manual(values=unique(df_data$celltype)) + # this is a kluge
    #geom_hline(yintercept=cutoff, linetype="dashed", color = "red") +
    scale_y_continuous("estimate", expand = c(0, 0)) +
