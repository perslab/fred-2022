...
colGeneWeights = "pkMs"
colGeneNames = "genes"
colModule = "module_meta_assoc"
colCellCluster = "cell_cluster_meta_assoc"
```
...
```{r}
path_geneMod_perslab <- paste0(dirWGCNA_tables,"liver_perslab_int_wgcna3_geneMod_merged_200325.csv.gz")
...
```{r}
vec_module_GERR <- rep(names(list_gaussResGO), sapply(list_gaussResGO, function(gaussRes) {gaussRes$selected_pathways_names %>% length}))
```
...
vec_p.value_fisher_GO_BH = p.adjust(vec_p.value_fisher, 
                                    method="BH",
                                    n=length(list_vec_modmetagenes)*(length(list_vec_genesetsGO)+length(list_vec_genesetsKEGG)))
...
```{r}
saveMeta(savefnc= fwrite, x = dt_resultsGO, file = paste0(dirWGCNA_tables, prefixData, "_", prefixRun, "_", flagDate,"_GERR_GOenrichmentResults_metaMod.csv"))
```
...
```{r}
saveMeta(savefnc= fwrite, x = dt_resultsKEGG, file = paste0(dirWGCNA_tables, prefixData, "_", prefixRun,  "_", flagDate,"_GERR_KEGGenrichmentResults_metaMod.csv"))
```
