...
```{r}
dt_condition_coef <- fread(paste0(dirWGCNA_tables, prefixData, "_mod_assoc_in_gerhard2018_BH_201111.csv"))
```
...
if (F) {
  ...
  seuratObj_sub <- subset(x=seuratObj, subset = cluster_perslab_coarse != "Erythrocytes" )

  # rename clusters
  
  
  vec_cluster_perslab <- seuratObj_sub$cluster_perslab
  ...
  seuratObj_sub$cluster_perslab <- vec_cluster_perslab_revised
  
  for (i in 1:4) {
    seuratObj_sub$cluster_perslab =   gsub(pattern=c("alpha-", "beta", "gamma-", "delta")[i], 
    replacement=c(alpha_unicode, Beta_unicode, gamma_unicode, delta_unicode)[i],
    x=seuratObj_sub$cluster_perslab)
  
  }
  
  for (i in 1:4) {
    seuratObj_sub$cluster_perslab_coarse =   gsub(pattern=c("alpha-", "beta", "gamma-", "delta")[i], 
    replacement=c(alpha_unicode, Beta_unicode, gamma_unicode, delta_unicode)[i],
    x=seuratObj_sub$cluster_perslab_coarse)
  
  }
  
  Idents(seuratObj_sub) <- "cluster_perslab"
  
  # re-run PCA and UMAP
  
  DefaultAssay(seuratObj_sub) = "integrated"
  seuratObj_sub %>% Seurat::RunPCA(object=., npcs=npcs, seed.use=randomSeed) %>% RunUMAP(object=., reduction="pca", seed.use=randomSeed, dims=1:npcs) -> seuratObj_sub
  
  saveRDS(seuratObj_sub, here("output","liver_perslab_int_seurat_7_SCTint_perslab_labels_seuratObj.RDS.gz"), compress="gzip")
...
if (F) {
  vec_cluster_perslab =unique(seuratObj_sub$cluster_perslab)
...
```{r}
if (F){
  vec_cluster_perslab_coarse =unique(seuratObj_sub$cluster_perslab_coarse)
...
```{r}
Idents(seuratObj_sub) <- "cluster_perslab_coarse"
```

```{r}
p <- UMAPPlot(seuratObj_sub, 
             #reduction = "umap",
...
<!-- names(vec_colors_full) = seuratObj_sub$cluster_perslab -->
...
<!-- names(vec_colors_full_1) = seuratObj_sub$cluster_perslab_coarse -->
...
<!-- ```{r} -->
<!-- dt_plot = data.table( -->
<!--   cluster_perslab = Idents(seuratObj_sub), -->
<!--   seuratObj_sub@reductions$umap@cell.embeddings) -->
<!-- ``` -->
...
<!-- names(vec_colors_cluster_perslab_full) <- levels(Idents(seuratObj_sub))[match(pdata$group, order(levels(Idents(seuratObj_sub))))] -->
...
```{r, include=F}
dt = data.table("cluster" = seuratObj_sub$cluster_perslab_coarse,
                "donor"= as.character(seuratObj_sub$sample_ID))
...
```{r}
dt_tmp = data.table("sample_ID" = seuratObj_sub@meta.data$sample_ID, "cluster_perslab_coarse" = seuratObj_sub@meta.data$cluster_perslab_coarse)
...
<!-- ```{r} -->
<!-- clusters = seuratObj_sub@meta.data$cluster_perslab_coarse %>% table %>% names  -->

<!-- Seurat::Idents(seuratObj_sub) <-seuratObj_sub@meta.data$cluster_perslab_coarse -->
...
Fig 1C : cluster marker genes violin plot
...
names(vec_markers) <-  sort(unique(seuratObj_sub$cluster_perslab_coarse))
```

```{r}
factor_cell_clust_group <- factor(sort(unique(seuratObj_sub$cluster_perslab_coarse)), ordered = T)
```
...
reorder cells in seuratObj_sub 

```{r}
Idents(seuratObj_sub) <- "cluster_perslab_coarse"
levels(x = seuratObj_sub) = sort(names(vec_markers), decreasing = T)
```

```{r}
#p2  <- VlnPlot(object=seuratObj_sub,
list_plot <- VlnPlot(object=seuratObj_sub,
                     assay="SCT_combat",
...
```{r}
p2Ai <- DimPlot(seuratObj_sub, 
             reduction = "umap",
             group.by="cluster_perslab_coarse", 
             seed=randomSeed, 
             label = F,
             #cols= vec_colors_cluster_perslab_coarse,
             cells.highlight =list(c(colnames(seuratObj_sub)[seuratObj_sub$cluster_perslab_coarse=="NK-like-cells"])),
...
seuratObj_macparland <- load_obj(path_seuratObj_macp)
```

```{r}
# includes Dendritic cells
path_dt_modPres_perslab_vs_macparland <- paste0(dirWGCNA_tables, "liver_perslab_int_seurat_7_wgcna3_cluster_perslab_coarse_vs_macparland_201116", "_df_preservationStats.csv")
...
```{r}
dt_resultsGO = fread(paste0(dirWGCNA_tables, prefixData, "_", prefixRun, "_", "201119","_GERR_GOenrichmentResults.csv"))
dt_resultsGO$source = "GO"

# KEGG
dt_resultsKEGG = fread(paste0(dirWGCNA_tables, prefixData, "_", prefixRun,  "_", "201119","_GERR_KEGGenrichmentResults.csv"))
dt_resultsKEGG$source = "KEGG"

# REACTOME
dt_resultsREACT = fread(paste0(dirWGCNA_tables, prefixData, "_", prefixRun,  "_","201119" ,"_GERR_REACTOMEenrichmentResults.csv"))
...
```{r}
size <- 30
mods_plot =c("Hepatic-stellate-cells_4", "Hepatocytes_4")
```

```{r}
for (mod_plot in mods_plot) {

  dt_resultsGO_revigo = fread(paste0(dirWGCNA_tables, prefixData, "_", prefixRun, "_", "201119","_GERR_GOenrichmentResults_revigo_", mod_plot,".csv"))
  ...
  dt_results_GERR_plot = rbindlist(list(dt_resultsGO_revigo, dt_resultsKEGG_plot, dt_resultsREACT_plot), use.names=T) # columns aren't in the same order in REVIGO, hence match by colnames
  ...
  ggsave(plot = p_GO, filename = paste0(dirWGCNA_plots,prefixData,"_", prefixRun,  "_fig2B_left_GO_barplot_", mod_plot, "_", params$date,".pdf"))
  ...
  module.plot <- ggraph(hub.plot, 
    ...
    #            labels = c(" \U00B1 1", " \U00B1 2", " \U00B1 3")
    #            ) +
    geom_node_text(aes(label = name), fontface="bold.italic", size=fontSize_label_lg, repel = T, color = "black") +
    guides(
  ...
  ggsave(module.plot,filename = paste0(dirWGCNA_plots,prefixData,"_", prefixRun, "_fig2B_right_network_", mod_plot, "_", params$date,".pdf"))#, width=8, height=round(nrow(dt_results_GERR_metaMod_plot_sub)/9.5,0)) 
}
...
colnames(dt_fibrosis)[c(1,3:ncol(dt_fibrosis))] = paste0("fibrosis_",colnames(dt_fibrosis)[c(1,3:ncol(dt_fibrosis))])

dt_lob_inflam = dt_condition_coef[condition == "lob_inflam"]
colnames(dt_lob_inflam)[c(1,3:ncol(dt_lob_inflam))] = paste0("lob_inflam_",colnames(dt_lob_inflam)[c(1,3:ncol(dt_lob_inflam))])
...
p = ggplot(data=dt_point, mapping=aes(lob_inflam_estimate_original, fibrosis_estimate_original, color=celltype, shape=significance)) + 
geom_point(size=6, alpha=0.9) + 
  ...
  #TODO
  # ggsave isn't rendering unicode - save from canvas
...
p = ggplot(data=dt_point, mapping=aes(lob_inflam_estimate_original, fibrosis_estimate_original, color=metamodule, shape=significance)) + 
  geom_point(size=6, alpha=0.9) + 
...
```{r}
Idents(seuratObj_sub) = "cluster_perslab_coarse"
levels(seuratObj_sub) = sort(names(vec_colors_cluster_perslab_coarse), decreasing = T)
```
...
list_p_vln_meta_sc = lapply(unique(dt_results_GERR_metaMod_plot$metamodule), function(metamod) {
  ...
  p_vln <- VlnPlot(object=seuratObj_sub,
                     assay="SCT_combat",
                     features = metamod,
                     pt.size = 0,#0.05,# 0.05,
                     cols=vec_colors_cluster_perslab_coarse,#vec_colors_markers,
...
```{r}
p_hep4 <- VlnPlot(object=seuratObj_sub,
                     assay="SCT_combat",
...
p_PIGR = FeaturePlot(seuratObj_sub, features="PIGR", reduction = "umap", slot = "data")
...
```{r}
seuratObj_sub_macs <- subset(seuratObj_sub, subset=cluster_perslab_coarse == "Macrophages")
```
...
```{r}
vec_colors_cluster_perslab_macs = vec_colors_cluster_perslab[names(vec_colors_cluster_perslab) %in% seuratObj_sub_macs$cluster_perslab]
```

```{r}
colnames(seuratObj_sub_macs@meta.data)[colnames(seuratObj_sub_macs@meta.data)=="deepskyblue"] <- "gerhard2018_7"
Idents(seuratObj_sub_macs) <- "cluster_perslab"
levels(seuratObj_sub_macs) <- rev(c("5-Macrophages", "6-Macrophages","11-Macrophages", "14-Macrophages"))
```
...
```{r}
list_plot_mac <- VlnPlot(object=seuratObj_sub_macs,
                     assay="SCT_combat",
...
```{r}
vec_modsPlot = c( "Macrophages_1","Macrophages_3","Macrophages_4")
vec_modsPlot_abbrev = c( "M-Mac-1","M-Mac-3","M-Mac-4")
```

```{r}
list_plot_mac_mods <- VlnPlot(object=seuratObj_sub_macs,
                     assay="SCT_combat",
...
<!-- ```{r} -->
<!-- list_p <- VlnPlot(object = seuratObj_sub_macs, -->
<!--               assay= "SCT_combat", -->
<!--               pt.size=0.05, -->
...
```{r}
Idents(seuratObj_sub) <- "cluster_perslab"
levels(seuratObj_sub) <- unique(seuratObj_sub$cluster_perslab)[order(gsub("\\d+-","", unique(seuratObj_sub$cluster_perslab)), decreasing=T)]
```

### Fig. 5 (G) 

```{r}
list_vlnplot_calgranulin <- VlnPlot(object=seuratObj_sub,
                     assay="SCT_combat",
...
## Fig. 6 (A): bar graph for inflammation and fibrosis (alt. a dot plot) for the significant bulk modules

```{r}

dt_fibrosis_gerhard = dt_condition_coef[condition == "fibrosis" & grepl("gerhard",module)]
colnames(dt_fibrosis_gerhard)[c(1,3:ncol(dt_fibrosis_gerhard))] = paste0("fibrosis_",colnames(dt_fibrosis_gerhard)[c(1,3:ncol(dt_fibrosis_gerhard))])

dt_lob_inflam_gerhard = dt_condition_coef[condition == "lob_inflam" & grepl("gerhard",module)]
colnames(dt_lob_inflam_gerhard)[c(1,3:ncol(dt_lob_inflam_gerhard))] = paste0("lob_inflam_",colnames(dt_lob_inflam_gerhard)[c(1,3:ncol(dt_lob_inflam_gerhard))])
...
## Fig. 6 (B): violin plots of bulk module expression in coarse clusters
...
if (F) {
  ...
  for (old_modname in dict_mods_gerhard$module) {
    print(old_modname)
    if (old_modname %in% colnames(seuratObj_sub@meta.data)) {
        colnames(seuratObj_sub@meta.data)[grep(old_modname, colnames(seuratObj_sub@meta.data))] <-  dict_mods_gerhard$module_abbrev[match(old_modname, dict_mods_gerhard$module)]
    }
  }
...
list_df_markers_cluster_perslab = safeParallel(
  fun=function(clust) {
    FindMarkers(object = seuratObj_sub, 
               slot="data",
               group.by = "cluster_perslab", 
               ident.1 = clust,
                only.pos = T,
  ...
  list_iterable = list("clust"=sort(unique(seuratObj_sub$cluster_perslab))), 
  simplify=F, 
  n_cores = 19, 
  timeout = 120)

names(list_df_markers_cluster_perslab) = sort(unique(seuratObj_sub$cluster_perslab))
...
list_df_markers_cluster_perslab_coarse = safeParallel(
  ...
  list_iterable = list("clust"=sort(unique(seuratObj_sub$cluster_perslab_coarse))), 
  simplify=F, 
  n_cores = 19, 
  timeout = 120)

names(list_df_markers_cluster_perslab_coarse) = sort(unique(seuratObj_sub$cluster_perslab_coarse))
...
```{r}
list_vec_topmarkers =   lapply(names(list_df_markers_cluster_perslab), function(clust_name) {
...
## metamodule UMAP plot (DROPPED)

```{r}

# Compute network kme values
vec_metamodules = unique(dt_geneMod_perslab$module_meta)
vec_metamodules = vec_metamodules[nchar(vec_metamodules)>0]
list_vec_metamod_modules <- lapply(vec_metamodules, function(metamodule) dt_geneMod_perslab[module_meta == metamodule,"module_renamed", with=F][[1]] %>% unique)
list_vec_mod_sizes <- lapply(list_vec_metamod_modules, function(vec_metamod_modules) {
  sapply(vec_metamod_modules, function(metamod_modules) length(dt_geneMod_perslab[module_renamed == metamod_modules, "genes", with=F][[1]])) %>% sort(., decreasing = T)
}) 
names(list_vec_mod_sizes) = vec_metamodules
# reorder by size
list_vec_metamod_modules = lapply(list_vec_mod_sizes, names)
names(list_vec_metamod_modules) = vec_metamodules
# take hubgen
```

```{r}
mat_scaled_embed_filter = mat_scaled_embed[,colnames(mat_scaled_embed) %in% unlist(list_vec_metamod_modules, T)]
```
...
# vec_hub_genes <- vec_mod_genes[order(vec_mod_kIMs, decreasing=T)]
# vec_hub_kIMs <- vec_mod_kIMs[order(vec_mod_kIMs, decreasing=T)]
df_hub.data <- cbind.data.frame(
  metamodule = rep(names(list_vec_mod_sizes), sapply(list_vec_mod_sizes, length)),
  module = unlist(list_vec_metamod_modules, F),
  size = unlist(list_vec_mod_sizes, F))

df_hub.data$colour = vec_colors_meta[match(df_hub.data$metamodule, names(vec_colors_meta))]

rownames(df_hub.data) = df_hub.data$module

```

add gene set enrichment info

```{r}
dt_resultsGO$minlog10_p.value_fisher_BH = -log10(dt_resultsGO$p.value_fisher_BH)

dt_resultsGO_plot_swne <- dt_resultsGO[,.(module, term =GO_term, minlog10_p.value_fisher_BH=-log10(p.value_fisher_BH),source)]
  
dt_resultsKEGG_plot_swne = dt_resultsKEGG[,.(module, term =KEGG_term, minlog10_p.value_fisher_BH=-log10(p.value_fisher_BH),source)]

dt_resultsREACT_plot_swne = dt_resultsREACT[,.(module, term =REACT_name, minlog10_p.value_fisher_BH=-log10(p.value_fisher_BH),source)]

dt_resultsGO_plot_swne = dt_resultsGO_plot_swne[order(-minlog10_p.value_fisher_BH)]
dt_resultsKEGG_plot_swne = dt_resultsKEGG_plot_swne[order(-minlog10_p.value_fisher_BH)]
dt_resultsREACT_plot_swne = dt_resultsREACT_plot_swne[order(-minlog10_p.value_fisher_BH)]

dt_results_GERR_plot_swne = rbindlist(list(dt_resultsGO_plot_swne, dt_resultsKEGG_plot_swne, dt_resultsREACT_plot_swne), use.names=T)
  
dt_results_GERR_plot_swne_dcast <- dcast.data.table(dt_results_GERR_plot_swne, formula = "term ~ module", value.var = "minlog10_p.value_fisher_BH", fun.aggregate = I, fill= 0)
...
seuratObj_embeddings_tmp <- Seurat::CreateSeuratObject(counts= CreateAssayObject(data=mat_scaled_embed_filter), assay="modEmbed")

seuratObj_embeddings_tmp <- Seurat::AddMetaData(seuratObj_embeddings_tmp, df_hub.data)


seuratObj_embeddings_tmp <- Seurat::ScaleData(
  object = seuratObj_embeddings_tmp,
  features=rownames(seuratObj_embeddings_tmp))
  
seuratObj_embeddings_tmp <- RunPCA(object = seuratObj_embeddings_tmp, 
                                   features=rownames(seuratObj_embeddings_tmp),
                                   assay= "modEmbed", seed.use = randomSeed, npcs = 40, verbose = F)

seuratObj_embeddings_tmp <- RunUMAP(object = seuratObj_embeddings_tmp, 
                                    assay= "modEmbed", 
                                    seed.use = randomSeed, 
                                    dims=1:40)
...
```{r}
mat_comb_embed_geneset <- mat_embed
```

```{r}
df_hub.data$UMAP_1 <- seuratObj_embeddings_tmp@reductions$umap@cell.embeddings[,1]
df_hub.data$UMAP_2 <- seuratObj_embeddings_tmp@reductions$umap@cell.embeddings[,2]
```


<!-- ```{r} -->
<!-- seuratObj_embeddings_tmp <- Seurat::FindNeighbors(seuratObj_embeddings_tmp, random.seed = randomSeed) -->
<!-- seuratObj_embeddings_tmp <- FindClusters(seuratObj_embeddings_tmp, resolution = 1.5, random.seed = randomSeed) -->
<!-- ``` -->

```{r}
p_meta <- DimPlot(seuratObj_embeddings_tmp, reduction = "umap", group.by = "metamodule", pt.size=df_hub.data$size/10, label = F)

p_meta <- p_meta +  ggrepel::geom_text_repel(
        direction    = "both",
        segment.color = "grey50",
        point.padding=0.1,
        #label.padding =0.1,
...
```{r,fig.width=10}
Seurat::FeaturePlot(object=seuratObj_sub, features="TREM2")
```
...
```{r}
DefaultAssay(object = seuratObj_sub) = "integrated"
```

```{r}
p <- Seurat::FeaturePlot(object=seuratObj_sub, features=vec_mods, blend = F, order = T)
...
```{r}
#list_seuratObj_sub[[1]]$macs_subclust <- factor(list_seuratObj_sub[[1]]$macs_subclust, levels = 0:6, ordered=T)
for (gene in vec_genes) {
  p <- Seurat::VlnPlot(object = seuratObj_sub, 
                       features = vec_genes,
                       assay = "SCT_combat", 
                       slot = "data", 
                       log=FALSE,
...
```{r}
FeaturePlot(object = seuratObj_sub, features = "VCAN")
```
...
```{r}
pdf(mat_moduleGeneIntersect, file = here("plots", paste0(prefixData, "_", prefixRun, "_supplfig_module_renamed_overlaps_",flagDate,".pdf")), width=max(10,ncol(mat_moduleGeneIntersect) %/% 3),
    height=max(10,ncol(mat_moduleGeneIntersect) %/% 3))
