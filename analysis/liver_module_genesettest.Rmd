...
```{r}
path_dt_metadataAssoc = here("output", paste0(prefixData, "_" , prefixRun, "_", prefixOut, "_", "191111_results.csv"))
...
```{r}
vec_allGenes <- readRDS("/projects/jonatan/pub-perslab/18-liver-fred/output/vec_allGenes.RDS")
```
...
```{r}
vec_module <- rep(names(list_gaussResGO), sapply(list_gaussResGO, function(gaussRes) {gaussRes$selected_pathways_names %>% length}))
```
