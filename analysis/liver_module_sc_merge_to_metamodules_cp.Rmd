...
## visualise module embeddings in UMAP space 


```{r}
mat_scaled_embed_filter = mat_scaled_embed[,!grepl("gerhard", colnames(mat_scaled_embed))]
```
...
```{r}
mat_scaled_embed_filter_svd = svd(x=mat_scaled_embed_filter, nv=10)

```


```{r}
embed_svd_v = mat_scaled_embed_filter_svd$v
```

```{r}
dt_svd = data.table(embed_svd_v)
#colnames(dt_umap) = c("UMAP_1", "UMAP_2")
dt_svd$module = colnames(mat_scaled_embed_filter) 
dt_svd$cell_cluster = gsub("_\\d$", "",colnames(mat_scaled_embed_filter))

# all.equal(dt_metadata$run_accession,dt_umap$run_accession)
# # [1] TRUE
# dt_umap = cbind(dt_umap, dt_metadata[,c("Diagnosis", "SEX", "Age", "BMI_surg")])
```

```{r}
ggplot(data = dt_svd, mapping=aes(x=V1, y=V2, color=cell_cluster)) +
...
```{r}
embed_umap = umap(d=embed_svd_v, preserve.seed = TRUE)
```

```{r}
dt_umap = data.table(embed_umap$layout)
colnames(dt_umap) = c("UMAP_1", "UMAP_2")
dt_umap$module = colnames(mat_scaled_embed_filter) 
#
for (i in 1:4) {
 dt_umap$module  =   gsub(pattern=c("alpha-", "beta", "gamma-", "delta")[i], 
  replacement=c(alpha_unicode, Beta_unicode, gamma_unicode, delta_unicode)[i],
  x=dt_umap$module )

}
dt_umap$cell_cluster = gsub("_\\d$", "",dt_umap$module )
...
```{r}
ggplot(data = dt_umap, mapping=aes(x=UMAP_1, y=UMAP_2, color=cell_cluster)) +
  geom_point(size=6, alpha=0.9) +
    scale_color_manual(
    values=vec_colors_cluster_perslab_coarse) 
```
...
<!--                                    features=rownames(seuratObj_embeddings_tmp), -->
<!--                                    assay= "modEmbed", seed.use = randomSeed, npcs = npcs, verbose = T) -->
...
<!-- ```{r} -->
<!-- seuratObj_embeddings_tmp <- FindNeighbors(seuratObj_embeddings_tmp, reduction = "pca", dims = 1:npcs, verbose = T) -->
<!-- seuratObj_embeddings_tmp <- FindClusters(seuratObj_embeddings_tmp, random.seed = randomSeed, verbose = T, resolution = 1) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- DimPlot(seuratObj_embeddings_tmp, reduction = "pca") -->
<!-- ``` -->
...
```{r}
pheatmap(mat = mat_moduleGeneIntersect)
```

```{r}
mat_moduleGeneIntersect %>% as.numeric %>% sort(.,TRUE) %>% head
...
```{r}
mat_scaled_embed_mod_sc <-  mat_scaled_embed[,!grepl("gerhard", colnames(mat_scaled_embed))]
```

```{r}

mat_cor <- cor(mat_scaled_embed_mod_sc, use="pairwise.complete.obs")
mat_cor[mat_cor<0] <- 0
...
```{r}
mat_cor = cor(mat_scaled_embed_mod_sc)
```
...
pdf(file = paste0("/projects/jonatan/pub-perslab/18-liver-wgcna/plots/", prefixData, "_", prefixRun, "_", prefixRun_WGCNA, "_corrplot_mod_metalabels_", flagDate,".pdf"), width = 20, height=26)
  ...
  corrplot::corrplot(mat_cor_renamed, 
                        method = "shade", 
                        col = col3(50),
                        type = "lower", 
                        diag = F, 
