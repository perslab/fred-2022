...
### original modules (not meta)

gerhard 2018 wgcna modules

```{r}
dt_geneMod_gerhard2018_merged$module_pres %>% unique -> vec_mods_gerhard2018

vec_mods_gerhard2018<- vec_mods_gerhard2018[!is.na(vec_mods_gerhard2018) & nchar(vec_mods_gerhard2018)>0]

mat_embed_gerhard2018 <- sapply(vec_mods_gerhard2018, function(mod) {

  # get gene weights
  condition = quote(dt_geneMod_gerhard2018_merged[[colModule]]==mod)
  vec_pkMs <- dt_geneMod_gerhard2018_merged[eval(condition), pkMs,]

  # normalize module gene weights to sum to 1
  vec_pkMs <- vec_pkMs/sum(vec_pkMs)

  # find corresponding rows of the expression matrix
  vec_idxRow <- match(dt_geneMod_gerhard2018_merged[eval(condition), ..colGeneNames, ][[1]], dt_datExpr[,gene])
...
rownames(mat_embed_gerhard2018) <- colnames(dt_datExpr)[-1]
...
```{r}
vec_allZero <- apply(X=mat_embed_scaled, MARGIN=2, FUN = function(x) all(x==0))
table(vec_allZero)
# FALSE
#  69
```
...
```{r}
dt_model_tmp <- data.table(dt_metadata, mat_embed_scaled[dt_metadata$geo_accession,])
```

```{r}

lapply(colnames(mat_embed_scaled), function(module) {
  formula_tmp <- formula(paste0(module, "~ fibrosis"))
  ##mat_model <- model.matrix(object = formula_tmp, data=dt_model_tmp)
  regr<-lm(formula = formula_tmp, dt_model_tmp)
  regr %>% broom::tidy() %>% as
}) -> list_tbl_tmp 

names(list_tbl_tmp) = colnames(mat_embed)
...
```{r}
dt_fibrosis_coef_gerhard2018 <- fread(paste0(dirWGCNA_tables, "liver_perslab_int_sc_bulk_modules_scaled_fibrosis_association_in_gerhard2018_200511.csv"))
```
...
check correlation of module betas for fibrosis in Moyal with module betas for lobular inflammation in gerhard 2018
...
## write out gene module results

```{r}

fwrite(x=dt_condition_coef, file=paste0(dirWGCNA_tables, prefixData, "_sc_bulk_modules_scaled_fibrosis_association_in_moylan2018_", flagDate,".csv"))

openxlsx::write.xlsx(x=dt_condition_coef,file = paste0(dirWGCNA_tables, prefixData, "_sc_bulk_modules_scaled_fibrosis_association_in_moylan2018_", flagDate,".xlsx"))
