...
```{r}
dt_geneMod_comb = data.table::rbindlist(list(dt_geneMod_perslab, dt_geneMod_gerhard2018))#, dt_geneMod_moylan2013))
```
...
vec_p.value_fisher_GO_BH = p.adjust(vec_p.value_fisher, 
                                    method="BH",
                                    n=length(list_vec_modGenes)*(length(list_vec_genesetsGO)+length(list_vec_genesetsKEGG)))
...
saveMeta(savefnc= fwrite, x = dt_resultsREACT, file = paste0(dirWGCNA_tables, prefixData, "_", prefixRun,  "_", flagDate,"_GERR_REACTOMEenrichmentResults.csv"))
