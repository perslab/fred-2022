...
```{r gaussianFit}
list_gaussResGO <- lapply(list_vec_modGenes_comb, function(vec_modGenes) {
...
vec_p.value_fisher_GO_BH = p.adjust(vec_p.value_fisher, 
                                    method="BH",
                                    n=length(list_vec_modGenes_comb)*(length(list_vec_genesetsGO)+length(list_vec_genesetsKEGG)))
...
```{r gaussianFit}
list_gaussResKEGG <- lapply(list_vec_modGenes_comb, function(vec_modGenes) {
  regression_selected_pathways(gene_input=vec_modGenes, 
...
```{r}
saveMeta(savefnc= fwrite, x = dt_resultsGO, file = paste0(dirWGCNA_tables, prefixData, "_", prefixRun, "_", flagDate,"_GERR_GOenrichmentResults.csv"))
```
...
```{r}
saveMeta(savefnc= fwrite, x = dt_resultsKEGG, file = paste0(dirWGCNA_tables, prefixData, "_", prefixRun,  "_", flagDate,"_GERR_KEGGenrichmentResults.csv"))
```
