...
## compare macrophage subcluster 2 in perslab and in MacParland

### process MacParland

```{r}
seuratObj_macParland <- SCTransform(object = seuratObj_macParland,  
                                    vars.to.regress = c("percent.mito"),
                                    seed.use = randomSeed) 
```

# subset the perslab macrophages

```{r}
seuratObj_sub <- subset(seuratObj, subset=cluster_perslab_coarse == celltype)
```
...
# vec_modsPlot = c("deepskyblue", unique(dt_geneMod_perslab[(nchar(module_fibrosis_gerhard2018)>0 | nchar(module_fibrosis_gerhard2018)>0) & cell_cluster == "Macrophages", module_renamed])) # top genes in "honeydew3" macrophage module
```
...
### look at gerhard2018_coral across all perslab 

```{r}
p_perslab_gerhard2018_mods <- VlnPlot(object = seuratObj, features = grep("^gerhard2018_", colnames(seuratObj@meta.data), value=T),  group.by = "cluster_perslab")
ggsave(plot=p_perslab_gerhard2018_mods, filename = here("plots", paste0(prefixData, "_", prefixRun, "_gerhard_mods_vlnplot_perslab_cluster_perslab.pdf")), height=40, width=40)
```
...
```{r}
DotPlot_timshel(object = seuratObj_macParland, 
                assay = "SCT", 
                slot = "counts", 
                do.scale = F, 
                genes.plot = vec_genesPlot, 
                cols.use = c("orangered", "blue"), 
                col.min = "blue", col.max = "orangered",  
...
The expression level is a bit confusing. Better stick with the violin jitter plots
...
```{r}
dt_plot_moccasin <- dt_mod_embeddings_gerhard[,.(run_accession,gerhard2018_moccasin)]
dt_plot_moccasin$Diagnosis <- dt_metadata_gerhard$Diagnosis[match(dt_plot_moccasin$run_accession, dt_metadata_gerhard$run_accession)]
```

#### lob inflam 

```{r}
dt_plot_moccasin_inflam_sub <- dt_plot_moccasin[Diagnosis %in% c("Lob Inflam 1", "Lob Inflam 2", "NORMAL") ]
```


```{r}
p11 <- ggplot(data = dt_plot_moccasin_inflam_sub, aes(x=gerhard2018_moccasin, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.5)
ggsave(plot=p11, filename = here("plots", paste0(prefixData, "_", prefixRun, "_moccasin_gerhard_inflam_histo.pdf")))
```
...
```{r}
dt_plot_moccasin_fibr_sub <- dt_plot_moccasin[grepl("Fibrosis|NORMAL", Diagnosis)]
#dt_plot_fibr_sub[,plot_color:=ifelse(Diagnosis == "Fibrosis4", "red", ifelse(Diagnosis == "Lob Inflam 1", "orange", "blue"))]
```

```{r}
p12 <- ggplot(data = dt_plot_moccasin_fibr_sub, aes(x=gerhard2018_moccasin, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.4)
ggsave(plot=p12, filename = here("plots", paste0(prefixData, "_", prefixRun, "_moccasin_gerhard_fibrosis_histo.pdf")))
```



### perslab_darkgoldenrod2 (hepatocytes)

```{r}
dt_plot_darkgoldenrod2 <- dt_mod_embeddings_gerhard[,.(run_accession,perslab_darkgoldenrod2)]
dt_plot_darkgoldenrod2$Diagnosis <- dt_metadata_gerhard$Diagnosis[match(dt_plot_darkgoldenrod2$run_accession, dt_metadata_gerhard$run_accession)]
```

#### lob inflam 

```{r}
dt_plot_darkgoldenrod2_inflam_sub <- dt_plot_darkgoldenrod2[Diagnosis %in% c("Lob Inflam 1", "Lob Inflam 2", "NORMAL") ]
```


```{r}
p11 <- ggplot(data = dt_plot_darkgoldenrod2_inflam_sub, aes(x=perslab_darkgoldenrod2, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.5)
ggsave(plot=p11, filename = here("plots", paste0(prefixData, "_", prefixRun, "_darkgoldenrod2_gerhard_inflam_histo.pdf")))
```
...
```{r}
dt_plot_darkgoldenrod2_fibr_sub <- dt_plot_darkgoldenrod2[grepl("Fibrosis|NORMAL", Diagnosis)]
#dt_plot_fibr_sub[,plot_color:=ifelse(Diagnosis == "Fibrosis4", "red", ifelse(Diagnosis == "Lob Inflam 1", "orange", "blue"))]
```

```{r}
p12 <- ggplot(data = dt_plot_darkgoldenrod2_fibr_sub, aes(x=perslab_darkgoldenrod2, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.4)
ggsave(plot=p12, filename = here("plots", paste0(prefixData, "_", prefixRun, "_darkgoldenrod2_gerhard_fibrosis_histo.pdf")))
```


### perslab_paleturquoise (hepatocytes)

```{r}
dt_plot_paleturquoise <- dt_mod_embeddings_gerhard[,.(run_accession,perslab_paleturquoise)]
dt_plot_paleturquoise$Diagnosis <- dt_metadata_gerhard$Diagnosis[match(dt_plot_paleturquoise$run_accession, dt_metadata_gerhard$run_accession)]
```

#### lob inflam 

```{r}
dt_plot_paleturquoise_inflam_sub <- dt_plot_paleturquoise[Diagnosis %in% c("Lob Inflam 1", "Lob Inflam 2", "NORMAL") ]
```


```{r}
p11 <- ggplot(data = dt_plot_paleturquoise_inflam_sub, aes(x=perslab_paleturquoise, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.5)
ggsave(plot=p11, filename = here("plots", paste0(prefixData, "_", prefixRun, "_paleturquoise_gerhard_inflam_histo.pdf")))
```
...
```{r}
dt_plot_paleturquoise_fibr_sub <- dt_plot_paleturquoise[grepl("Fibrosis|NORMAL", Diagnosis)]
#dt_plot_fibr_sub[,plot_color:=ifelse(Diagnosis == "Fibrosis4", "red", ifelse(Diagnosis == "Lob Inflam 1", "orange", "blue"))]
```

```{r}
p12 <- ggplot(data = dt_plot_paleturquoise_fibr_sub, aes(x=perslab_paleturquoise, fill=Diagnosis)) + geom_histogram(bins=50, alpha = 0.4)
ggsave(plot=p12, filename = here("plots", paste0(prefixData, "_", prefixRun, "_paleturquoise_gerhard_fibrosis_histo.pdf")))
```
