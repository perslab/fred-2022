...
add metamodule cell embeddings to seurat metadata
...
retrieve module embeddings from seuratObj

```{r}
mat_scaled_embed_sc = seuratObj@meta.data[,colnames(seuratObj@meta.data) %in% unique(dt_geneMod_perslab$module_renamed)]
```
...
```{r}
vec_mods <- dt_geneMod_perslab[["module_renamed"]]
names(vec_mods) = dt_geneMod_perslab[["module_meta"]]
vec_mods <- vec_mods[!duplicated(vec_mods)]
vec_mods <- vec_mods[!is.na(vec_mods) & nchar(vec_mods)>0]
vec_mods <- vec_mods[!duplicated(vec_mods)]
#vec_mods = gsub("-","_", vec_mods)
names(vec_mods) %>% unique %>% sort -> vec_mods_meta
```
...
mat_embed_meta = sapply(vec_mods_meta, function(mod_meta) {
  ...
  mat_tmp = mat_scaled_embed_sc[,vec_mods_in_mod_meta]
...
```{r}
seuratObj <- AddMetaData(object=seuratObj, metadata = data.frame(row.names=rownames(seuratObj@meta.data), mat_embed_meta_rescaled))
```
